<template>
  <div class="container">
    ...
    <div class="popup-content">
      <p class="popup-title">恭喜您，团购成功，请填写收货地址</p>
      <div class="row">
        <span class="label">收货人：</span>
        <input maxlength="25" type="text" v-model="formData1.receiver" />
      </div>
      <div class="row">
        <span class="label">手机号码：</span>
        <input maxlength="11" type="number" v-model="formData1.mobile" />
      </div>
      <div class="row">
        <select @change="changeProvince" ref="cmbProvince" v-model="formData1.province">
          <option class="select-placeholder" disabled selected value>请选择省...</option>
          <option :value="item.name" v-for="item in provinceList">{{item.name}}</option>
        </select>
      </div>
      <div class="row">
        <select ref="cmbCity" v-model="formData1.city" @change="changeCity">
          <option class="select-placeholder" disabled selected value>请选择市...</option>
          <option :value="item.name" v-for="item in cityList">{{item.name}}</option>
        </select>
      </div>
      <div class="row">
        <select ref="cmbArea" v-model="formData1.area">
          <option class="select-placeholder" disabled selected value>请选择区/县...</option>
          <option :value="item" v-for="item in areaList">{{item}}</option>
        </select>
      </div>
      <textarea cols="30" maxlength="120" minlength="5" placeholder="请填写详细地址：如街道、门牌号、小区、楼栋号、单元室等" rows="10" v-model="formData1.detailAddress"></textarea>
      <div :class="[{'btn-save-active':isActive},'btn-save']" @click="funSubmit">保存</div>
      <div @click="closePopup" class="btn-close"></div>
    </div>
    <v-dialog :dialogText="dialogText" @closeDialog="closeDialog" v-if="showDialog"></v-dialog>
  </div>
</template>
<script>
...
{name:'澳门', cityList:[  
  ...
  computed: {
    isActive() {
      if (
        this.formData1.receiver !== '' &&
        this.formData1.mobile !== '' &&
        this.formData1.province !== '' &&
        this.formData1.city !== '' &&
        this.formData1.area !== '' &&
        this.formData1.detailAddress !== ''
      ) {
        return true
      } else {
        return false
  ...
  mounted() {
    this.isFadeIn = true
    // this.formData1=this.clone(this.formData)
  },
  beforeDestroy() {
    this.isFadeIn = false
  },
  methods: {
    ...
    updataCity() {
      this.provinceList.forEach((item, index) => {
        ...
        if (item.name == this.formData1.province) {
          this.cityList = item.cityList
          //this.city = this.formData.city;
          this.$emit('updataCityFinished')
        }
      })
      //初始化默认值
      this.formData1.city = this.cityList[0].name
    },
    updataSub() {
      this.cityList.forEach((item, index) => {
        if (item.name == this.formData1.city) {
          this.areaList = item.areaList
          this.$emit('updataSubFinished')
        }
      })
      this.formData1.area = this.areaList[0]
    },
    closePopup() {
       this.formData1.receiver = ''
      this.formData1.mobile = ''
    ...
    funSubmit() {
      if (
        ...
        this.dialogText='请输入正确的手机号'
        return
      } 
      if(!this.formData1.receiver || !this.formData1.mobile || !this.formData1.province || !this.formData1.city || !this.formData1.area || !this.formData1.detailAddress){this.showDialog = true
       this.showDialog = true
       this.dialogText='请填写全部地址'
       return
      }
      if(this.formData1.detailAddress.length<6){
        this.showDialog = true
       this.dialogText='详细地址不能少于五个字'
       return
      }
       this.$emit('submit')
...
.popup-content {
  ...
  align-items: center;
  padding: 0.63rem 0.61rem 1.24rem 0.6rem;
  .popup-title {
    color: #282828;
    font-size: 0.4rem;
  ...
  .row {
    ...
    i {
      ...
      top: 0.25rem;
      background-color: #fff;
      background-image: url('~assets/group_buy/img/icon-select.png');
      background-size: cover;
      background-repeat: no-repeat;
    }
    .label {
  ...
  .btn-save {
    background-image: url('~assets/group_buy/img/btn-save-static.png');
    background-size: cover;
    background-repeat: no-repeat;
    width: 5.85rem;
    height: 1.07rem;
  ...
  .btn-save-active {
    background-image: url('~assets/group_buy/img/btn-save-active.png') !important;
  }
  .btn-close {
    width: 0.75rem;
    height: 0.75rem;
